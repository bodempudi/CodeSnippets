...
namespace NotepadPlusPlusPlugin.Formatting.Policies
{
        ...
        ILayoutPolicy
    {
            ...
            LayoutContext layoutContext)
        {
            ...
            /*
             * Temporary THROW diagnostics.
             *
             * Use LAYOUT_RULE because this category is already
             * included in the generated formatter report.
             */
            if (context.Node is ThrowStatement)
            {
                layoutContext.Diagnostics.Add(
                    "LAYOUT_RULE",
                    "DEBUG_THROW: Reached StatementLayoutPolicy. " +
                    "Node=" +
                    ...
                    "Statements"))
            {
                if (context.Node is ThrowStatement)
                {
                    layoutContext.Diagnostics.Add(
                        "LAYOUT_RULE",
                        "DEBUG_THROW: Returned because relationship " +
                        "is not Statements[n]. " +
                        "Relationship=" +
                        context.Relationship);
                }
            ...
            if (!(context.Parent is StatementList))
            {
                if (context.Node is ThrowStatement)
                {
                    layoutContext.Diagnostics.Add(
                        "LAYOUT_RULE",
                        "DEBUG_THROW: Returned because parent " +
                        "is not StatementList. Parent=" +
                        (context.Parent == null
                            ? "NULL"
                            : context.Parent.GetType().Name));
                }
            ...
            if (context.Node is ThrowStatement)
            {
                layoutContext.Diagnostics.Add(
                    "LAYOUT_RULE",
                    "DEBUG_THROW: Adding StatementLayoutPolicy " +
                    "instruction. Indent=" +
                    indent +
                    ", TokenIndex=" +
                    context.Node.FirstTokenIndex);
