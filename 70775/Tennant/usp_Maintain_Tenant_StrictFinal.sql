...
GO
	...
	-- Notes          : Strict formatting applied. UPSERT reuses logic. No space after BEGIN or before END.
	---------------------------------------------------------------------------------------------------------------------------------------
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			AND NOT EXISTS (
				SELECT 1 FROM InMem.Tenant WITH (NOLOCK) WHERE TenantID = @I_TenantID
			)
			BEGIN
				SELECT
					StatusCode = CONVERT(BIT, 1),
